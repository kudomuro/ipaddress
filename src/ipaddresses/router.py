...
async def add_specific_ipaddresses(new_ip: CreateIpaddress, user_auth = Depends(current_user), session: AsyncSession = Depends(get_async_session)):
    ...
    query_check_ip = await session.execute(select(ipaddress).filter(ipaddress.c.ip == enter_ip))
    ...
    async def find_count_child(parent_network_res):
        query_count_child = (
           select(ipaddress.c.parent_id)
            .filter(ipaddress.c.parent_id == parent_network_res)
        )
        # query_count_child = ("SELECT COUNT(*) FROM ipaddress WHERE parent_id = '{parent_network_res}' GROUP BY parent_id;")
        result = await session.execute(query_count_child)
        count_result = result.all()
        # print(query_count_child)
        return count_result
    
    parent_network = await find_parent(enter_ip)
    parent_network_res = parent_network[0]
    parent_network_mask = parent_network[3]

    # if (not check_ip):
    #     count_child = await find_count_child(parent_network_res)
    #     count_child_res = len(count_child)
    #     percent_use = (count_child_res*100)/(2**(32-parent_network_mask)-2)        
    #     if (percent_use<100):
    ...
    new_ip.parent_id=parent_network_res
    print(new_ip.ip.prefixlen)
    # print('Количество: ',count_child_res)
